...
private:
    ...
    {
        tr::for_each(candidates_, [](Candidate& candidate) { --candidate.replication; });
    }
    ...
    {
        return tr::find_if(candidates_, [piece](auto const& c) { return c.piece == piece; });
    }

    [[nodiscard]] constexpr CandidateVec::iterator find_by_block(tr_block_index_t const block)
    {
        return tr::find_if(candidates_, [block](auto const& c) { return c.block_belongs(block); });
    }
